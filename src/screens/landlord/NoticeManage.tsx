...
export default function NoticeManageScreen({ navigate }: { navigate: LandlordNavigate }) {
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: IVORY }}>
      <NavHeader
        ...
        rightEl={
          <button onClick={() => navigate("notice-write")} className="ml-auto px-3 py-1.5 rounded-xl text-xs font-extrabold flex-shrink-0 active:scale-95" style={{ background: ORANGE, color: NAVY }}>
            + 공지 작성
          </button>
        }
      />
      <div className="flex-1 overflow-y-auto px-4 py-4">
        <p className="text-xs font-medium mb-3" style={{ color: NAVY, opacity: 0.45 }}>
          여기서 작성한 공지는 세입자 앱의 &lsquo;게시판 &gt; 공지 게시판&rsquo;에 그대로 표시됩니다.
        </p>
